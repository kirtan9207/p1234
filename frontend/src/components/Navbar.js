...
export default function Navbar() {
  ...
  const navLink = (to, label) => (
    <Link
      to={to}
      className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive(to) ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'}`}
    >
      {label}
    </Link>
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden md:flex items-center gap-3">
            {!user ? (
              <>
                ...
                <Link to="/auth" className="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors">Get Started</Link>
              </>
            ) : (
        ...
        {menuOpen && (
          <div className="md:hidden py-3 border-t border-slate-100 space-y-1">
            ...
            {!user && <Link to="/auth" className="block px-3 py-2 text-sm font-medium text-indigo-600 hover:bg-indigo-50 rounded-lg" onClick={() => setMenuOpen(false)}>Sign In / Register</Link>}
